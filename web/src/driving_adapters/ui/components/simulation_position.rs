...
pub fn SimulationPosition(
    ...
    view! {
        <section class="relative flex h-full min-h-0 flex-col border border-border bg-surface" aria-label="Simulation position draft">
            <div class="panel-header"><h2 class="text-sm font-semibold">"Strategy builder"</h2><span class="text-[0.625rem] font-semibold uppercase tracking-wider text-level-special">"Browser draft"</span></div>
            <div class="p-3"><label class="sr-only" for="simulation-strategy">"Strategy"</label><select id="simulation-strategy" class="min-h-9 w-full rounded border border-border bg-canvas px-3 text-xs text-text-primary" disabled><option>{strategy_name}</option></select></div>
                ...
                <table class="w-full min-w-[25.5rem] table-fixed text-[0.6875rem]">
                    ...
                    <tbody class="divide-y divide-border numeric">
                        {move || rows.get().into_iter().map(|leg| {
                            ...
                            let cash_flow_class = if quantity < 0 { "px-1.5 py-3 text-right font-semibold text-finance-positive" } else { "px-1.5 py-3 text-right font-semibold text-negative-text" };
                            ...
                            view! {
                                <tr class="hover:bg-state-hover">
                                    ...
                                    <td class="px-1.5 py-2 text-right text-text-primary"><span class="inline-flex items-center justify-end gap-1"><span class="min-w-4 text-right">{quantity.unsigned_abs()}</span><span class="inline-flex flex-col gap-px"><button type="button" class="flex h-3.5 w-4 items-center justify-center rounded-sm border border-border text-[0.625rem] leading-none text-text-secondary hover:bg-state-hover hover:text-text-primary" aria-label="Increase quantity" on:click=move |_| adjust_quantity(rows, &increase_key, 1)>"+"</button><button type="button" class="flex h-3.5 w-4 items-center justify-center rounded-sm border border-border text-[0.625rem] leading-none text-text-secondary hover:bg-state-hover hover:text-text-primary" aria-label="Decrease quantity" on:click=move |_| adjust_quantity(rows, &decrease_key, -1)>"−"</button></span></span></td>
...
fn format_cash_flow(quantity: i32, instrument: &str, price: &str) -> String {
    ...
    let Ok(unit_price) = normalized.parse::<f64>() else { return "—".to_owned(); };
    let multiplier = if instrument.eq_ignore_ascii_case("CALL") || instrument.eq_ignore_ascii_case("PUT") { 100.0 } else { 1.0 };
...
fn adjust_quantity(rows: RwSignal<Vec<DraftLeg>>, key: &str, magnitude_delta: i32) {
    rows.update(|legs| {
        if let Some(index) = legs.iter().position(|leg| leg.key == key) {
            ...
            legs[index].quantity = sign * (magnitude + magnitude_delta).max(1);
        }
