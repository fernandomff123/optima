...
pub fn Topbar() -> impl IntoView {
    view! {
        <header class="flex h-12 items-center justify-between gap-4 border-b border-border bg-canvas px-4 sm:px-5">
            ...
            <label class="relative hidden min-w-0 max-w-[26rem] flex-1 lg:block">
                ...
                <input class="h-8 w-full rounded border border-border bg-surface py-0 pl-9 pr-3 text-xs text-text-secondary placeholder:text-text-muted-readable disabled:cursor-not-allowed disabled:opacity-80" type="search" placeholder="Search ticker, company, ETF or index…" disabled />
            </label>
